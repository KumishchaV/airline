...
public class UnBlockAddCrewsCommand implements Command {

	private static final Logger log = Logger.getLogger(UnBlockAddCrewsCommand.class);
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			...
			if (null == user) {
				...
				flightHasEmployee = flightHasEmployeeService.viewPlaneByFlightId(flightId);

				if (flightHasEmployee != null) {
					session.setAttribute("flightHasEmployee", flightHasEmployee);
				}

				employees = flightHasEmployeeService.viewAllFreeEmployee(utilDate);
